...
from config import OPENAI_API_KEY, JUDGE_MODEL, HUMAN_LABELS_PATH, get_openai_client, LLM_MODEL
...
'''
    ...
    resp = client.chat.completions.create(
        model=JUDGE_MODEL or LLM_MODEL,
        messages=[
    ...
    if winner not in {"A", "B", "tie"}:
        winner = "tie"
    scores = data.get("scores") or {"A": 0.0, "B": 0.0}
    # Clamp scores to [0, 1]
    scores = {
        "A": max(0.0, min(1.0, float(scores.get("A", 0.0)))),
...
def _judge_binary_label(question: str, model_answer: str, ground_truth: str = "") -> int:
    """Score a single answer as good(1)/bad(0) via direct quality judgment."""
    gt_block = f"\nGround truth (tham chiếu):\n{ground_truth}\n" if ground_truth else ""
    prompt = f'''Bạn là chuyên gia chấm chất lượng câu trả lời RAG HR policy.
...
Quy tắc chấm (bắt buộc tuân thủ):
- So sánh câu trả lời với ground truth (nếu có) và kiến thức chính sách HR hiện hành (v2024).
- label=1 nếu ý chính ĐÚNG (số liệu/điều kiện khớp, không dùng policy hết hiệu lực). Được phép ngắn gọn hơn ground truth.
- label=0 nếu SAI số liệu, dùng policy cũ (vd 12 ngày phép thay vì 15), thiếu điều kiện quan trọng, hoặc trả lời ngược lại chính sách.
- Đừng quá khắt khe về wording — chỉ cần đúng nội dung.

Chỉ trả JSON: {{"label": 0 hoặc 1, "reasoning": "giải thích ngắn"}}
'''
    client = get_openai_client()
    resp = client.chat.completions.create(
        model=JUDGE_MODEL or LLM_MODEL,
        messages=[
            {"role": "system", "content": "Chấm điểm RAG. Chỉ trả JSON với label 0 hoặc 1. Ưu tiên đúng nội dung hơn wording."},
...
if __name__ == "__main__":
    ...
    # Extra pairs for bias report (≥5)
    extra_pairs = [
        (
            "Thưởng Tết tối thiểu là bao nhiêu?",
            "Nhân viên chính thức từ 6 tháng trở lên được thưởng tối thiểu 1 tháng lương.",
            "Thưởng Tết khoảng vài triệu tùy công ty.",
        ),
        (
            "Nhân viên thử việc có được nghỉ phép năm không?",
            "Không được nghỉ phép năm; nếu cần nghỉ thì xin nghỉ không lương.",
            "Thử việc được nghỉ phép như nhân viên chính thức.",
        ),
        (
            "VPN cá nhân khi WFH có được không?",
            "Không được dùng VPN cá nhân; phải dùng VPN WireGuard của công ty.",
            "Được dùng NordVPN nếu kết nối ổn định.",
        ),
        (
            "Muốn mua thiết bị 55 triệu cần ai phê duyệt?",
            "Vượt 50 triệu nên cần CEO phê duyệt.",
            "Giám đốc phòng ban phê duyệt là đủ.",
        ),
    ]
    for q_e, a_good, a_bad in extra_pairs:
        print(f"\nJudging: {q_e[:50]}...")
        pairwise_results.append(swap_and_average(q_e, a_good, a_bad))
    ...
    # Map ground truths from test set for fairer judging
    gt_by_qid: dict[int, str] = {}
    try:
        with open("test_set_50q.json", encoding="utf-8") as f:
            for item in json.load(f):
                gt_by_qid[item["id"]] = item.get("ground_truth", "")
    except FileNotFoundError:
        pass

    judge_labels = []
    for item in human_data:
        gt = gt_by_qid.get(item["question_id"], "")
        label = _judge_binary_label(item["question"], item["model_answer"], gt)
        ...
        json.dump(
            {
                ...
                "bias": bias,
                "pairwise": [asdict(r) for r in pairwise_results],
            },
            f,
            ensure_ascii=False,
            indent=2,
