...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    for a, pq in zip(answers, per_q):
        # Support both EvalResult objects and plain dicts
        if isinstance(pq, dict):
            f, ar = pq.get("faithfulness", 0.0), pq.get("answer_relevancy", 0.0)
            cp, cr = pq.get("context_precision", 0.0), pq.get("context_recall", 0.0)
        else:
            f, ar = pq.faithfulness, pq.answer_relevancy
            cp, cr = pq.context_precision, pq.context_recall
        results.append(RagasResult(
            ...
            faithfulness=float(f), answer_relevancy=float(ar),
            context_precision=float(cp), context_recall=float(cr),
        ))
    return results
